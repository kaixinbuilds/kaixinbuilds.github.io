...
automatically. `build.py` also writes an empty `.nojekyll`, which switches GitHub Pages out
of Jekyll and into serving the files as they are. Keep it: every page here is finished HTML,
Jekyll has nothing to add, and a failed Jekyll build takes the deploy down **silently** —
Pages carries on serving the last good commit, so the site just quietly stops updating. Static titles, descriptions, Open Graph and canonical tags are written into

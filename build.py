#!/usr/bin/env python3
"""Regenerate the site's HTML pages from one shared shell.

Every page carries the same head, header and footer. Holding that in one
place means a nav change is a single edit rather than five, and no page can
quietly drift out of step with the others.

    python3 build.py

publish.sh runs this before every commit, so the HTML in the repo is always
this file's output. Edit this file, never the .html files: they are
overwritten on the next build.
"""
import json, pathlib, re

BANNER = "<!-- Generated by build.py. Edit build.py, not this file. -->"

# Google Analytics 4 measurement id, e.g. "G-3RT85MXN6L". Leave it empty and
# no analytics markup is emitted at all: no script tag, no request, nothing to
# consent to. Paste the id here and run ./publish.sh to switch it on for every
# page at once. Get it from the GA4 property under Admin > Data streams.
GA_MEASUREMENT_ID = "G-EDXEQ531WL"

GA_SNIPPET = """
<!-- Google Analytics 4 -->
<script async src="https://www.googletagmanager.com/gtag/js?id=__GA_ID__"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
  gtag('config', '__GA_ID__');
</script>
"""

# (file, i18n key for the nav label, fallback text)
NAV = [
    ("work.html", "nav.work", "Work"),
    ("connect.html", "nav.connect", "Connect"),
    ("practice.html", "nav.approach", "Practice"),
    ("contact.html", "nav.contact", "Contact"),
]

CURRENT = ' aria-current="page"'

HEAD = """\
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title data-i18n="__TITLE_KEY__">__STATIC_TITLE__</title>
<meta name="description" data-i18n="site.description" data-i18n-attr="content" content="__STATIC_DESC__">
<link rel="canonical" href="__CANONICAL__">

<meta property="og:type" content="website">
<meta property="og:site_name" content="kaixinbuilds">
<meta property="og:title" content="__STATIC_TITLE__">
<meta property="og:description" content="__STATIC_DESC__">
<meta property="og:url" content="__CANONICAL__">
<meta property="og:image" content="__SHARE_IMAGE__">
<meta name="twitter:card" content="summary_large_image">
<meta name="color-scheme" content="light">
<meta name="theme-color" content="#1E2749">

<!-- Spectral for headings and epigraphs, Inter for body and UI. Chinese
     resolves to the system serif (Songti SC / Source Han Serif): a webfont
     with CJK coverage would cost megabytes for one page. -->
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&family=Spectral:ital,wght@0,400;0,600;1,400&display=swap" rel="stylesheet">

__GA__<link rel="stylesheet" href="style.css">
<link rel="icon" href="favicon.png" sizes="48x48">
<link rel="apple-touch-icon" href="apple-touch-icon.png">
</head>
<body>

<a class="skip-link" href="#main">Skip to content</a>

<header class="site-header">
  <div class="wrap">
    <div class="header-inner">
      <a class="wordmark" href="index.html"__HOME__>
        <img class="mark" src="favicon.png" alt="" width="22" height="22" decoding="async">
        <span class="wordmark-text" id="wordmark-text">kaixin<em>builds</em></span>
      </a>

      <nav class="site-nav" aria-label="Primary">
__NAV__      </nav>

      <!-- Three modes, like the game: Chinese, both, English. The labels are
           language names, so they are never themselves translated. -->
      <div id="lang-switch" class="lang-switch" role="group" aria-label="Language / 语言">
        <button type="button" data-lang="zh" lang="zh-Hans">华文</button>
        <button type="button" data-lang="both" lang="zh-Hans">双语</button>
        <button type="button" data-lang="en" lang="en">English</button>
      </div>
    </div>
  </div>
</header>

"""

TAIL = """\


<script src="script.js"></script>
</body>
</html>
"""

SITE = "https://kaixinbuilds.github.io"
SHARE_IMAGE = SITE + "/assets/social-card.jpg"

# (static <title>, static description) shipped in the HTML before any script
# runs. The language toggle still rewrites the visible title afterwards.
META = {
    "index.html": ("Chun Kai Xin - Learning Designer",
                   "Classroom-grounded Chinese Language learning tools, designed and built by "
                   "a practising Singapore teacher."),
    "work.html": ("Work - Chun Kai Xin",
                  "VocabSummit, a Secondary Chinese Language vocabulary platform, plus other "
                  "tools built for specific classroom problems."),
    "connect.html": ("Connect - Chun Kai Xin",
                   "Workshops, sharings and write-ups for other teachers, including two "
                   "SgLDC badges awarded by MOE's Educational Technology Division."),
    "practice.html": ("Practice - Chun Kai Xin",
                      "How a classroom tool actually gets designed: a short loop where the "
                      "deciding step is students using it in a real lesson."),
    "2026AIinSecCLNLC.html": ("2026 AI in Sec CL NLC - Chun Kai Xin",
                              "Resources for the 28 August 2026 AI in Sec CL NLC workshop: the five pieces of technology behind VocabSummit, and a starter game to adapt for your own unit."),
    "contact.html": ("Contact - Chun Kai Xin",
                     "Get in touch about adapting any of these tools, or about building "
                     "something together."),
}

PAGES = {
    "index.html": ("page.home.title", """\
  <section class="hero" id="top">
    <div class="wrap">
      <!-- The couplet VocabSummit is named from. Both languages always show:
           the pairing itself is the content, not a translation of it. -->
      <p class="epigraph">
        <span class="zh" lang="zh-Hans">不满是向上的车轮</span>
        <span class="en" lang="en">Discontent is the wheel that carries us forward.</span>
        <span class="src" data-i18n="epigraph.source">鲁迅</span>
      </p>

      <p class="eyebrow" data-i18n="brand.full">kaixinbuilds</p>
      <h1 class="hero-name" data-i18n="hero.name">Chun Kai Xin</h1>
      <p class="hero-tagline" data-i18n="hero.tagline"></p>
      <p class="hero-intro" data-i18n="hero.intro"></p>
      <p class="hero-actions">
        <a class="btn btn-primary" href="work.html" data-i18n="hero.ctaWork">See my work</a>
        <a class="btn btn-ghost" href="contact.html" data-i18n="hero.ctaContact">Get in touch</a>
      </p>
    </div>
  </section>"""),
    "work.html": ("page.work.title", """\
  <section class="section" id="work">
    <div class="wrap">
      <h1 class="page-title" data-i18n="nav.work">Work</h1>
      <p class="page-sub" data-i18n="work.intro"></p>

      <div class="work-layout">
        <!-- An index of everything, and one project open at a time. Without
             JavaScript every panel simply shows, stacked. -->
        <nav id="project-nav" class="project-nav" aria-label="Projects"></nav>
        <div id="project-detail" class="project-detail"></div>
      </div>
    </div>
  </section>"""),
    "connect.html": ("page.connect.title", """\
  <section class="section" id="connect">
    <div class="wrap">
      <h1 class="page-title" data-i18n="connect.title">Connect</h1>

      <div class="work-layout">
        <!-- Built by script.js from connect.json, same shape as the Work index. -->
        <nav id="connect-nav" class="project-nav" aria-label="Connect"></nav>
        <div id="connect-list"></div>
      </div>
    </div>
  </section>"""),
    "practice.html": ("page.approach.title", """  <section class="section" id="approach">
    <div class="wrap wrap-narrow">
      <h1 class="page-title" data-i18n="approach.title">Practice</h1>

      <!-- A couplet for the philosophy, in parallel with the one that names
           the flagship project on the home page. -->
      <p class="epigraph">
        <span class="zh" lang="zh-Hans">纸上得来终觉浅，绝知此事要躬行</span>
        <span class="en" lang="en">What is learnt on paper stays shallow.
          To truly know a thing, you have to do it yourself.</span>
        <span class="src">陆游《冬夜读书示子聿》</span>
      </p>

      <p class="lede" data-i18n="approach.leadA"></p>

      <!-- The five stages of the design thinking cycle, named and nothing
           more: the heading is the stage, and what happened while building
           VocabSummit is answered underneath in the fewest words that carry
           it. The point counts differ by stage because the stages differ; a
           stage is not padded to match its neighbours. -->
      <ol class="loop">
        <li>
          <h2 data-i18n="approach.loop1Title"></h2>
          <ul class="loop-points">
            <li data-i18n="approach.loop1P1"></li>
            <li data-i18n="approach.loop1P2"></li>
            <li data-i18n="approach.loop1P3"></li>
          </ul>
        </li>
        <li>
          <h2 data-i18n="approach.loop2Title"></h2>
          <ul class="loop-points">
            <li data-i18n="approach.loop2P1"></li>
            <li data-i18n="approach.loop2P2"></li>
            <li data-i18n="approach.loop2P3"></li>
            <li data-i18n="approach.loop2P4"></li>
          </ul>
        </li>
        <li>
          <h2 data-i18n="approach.loop3Title"></h2>
          <ul class="loop-points">
            <li data-i18n="approach.loop3P1"></li>
            <li data-i18n="approach.loop3P2"></li>
            <li data-i18n="approach.loop3P3"></li>
            <li data-i18n="approach.loop3P4"></li>
          </ul>
        </li>
        <li>
          <h2 data-i18n="approach.loop4Title"></h2>
          <ul class="loop-points">
            <li data-i18n="approach.loop4P1"></li>
            <li data-i18n="approach.loop4P2"></li>
            <li data-i18n="approach.loop4P3"></li>
            <li data-i18n="approach.loop4P4"></li>
          </ul>
        </li>
        <li>
          <h2 data-i18n="approach.loop5Title"></h2>
          <ul class="loop-points">
            <li data-i18n="approach.loop5P1"></li>
            <li data-i18n="approach.loop5P2"></li>
            <li data-i18n="approach.loop5P3"></li>
          </ul>
        </li>
      </ol>

      <p class="loop-close" data-i18n="approach.loopClose"></p>

      <!-- The loop above is how the thing gets made. This is what it is aimed
           at, and it is the spine of the SPMC sharing: Self-Determination
           Theory turns "make vocabulary fun" into three answerable questions,
           and every feature listed here exists to answer one of them. Each
           need opens with the student sentence it is a reply to. -->
      <section class="needs-block" aria-labelledby="needs-title">
        <h2 class="section-title" id="needs-title" data-i18n="approach.needsTitle"></h2>
        <p class="lede" data-i18n="approach.needsLead"></p>

        <div class="needs">
          <article class="need">
            <p class="need-name" data-i18n="approach.need1Name"></p>
            <p class="need-voice" data-i18n="approach.need1Voice"></p>
            <h3 data-i18n="approach.need1Q"></h3>
            <ul class="loop-points need-points">
              <li data-i18n="approach.need1P1"></li>
              <li data-i18n="approach.need1P2"></li>
              <li data-i18n="approach.need1P3"></li>
              <li data-i18n="approach.need1P4"></li>
            </ul>
          </article>
          <article class="need">
            <p class="need-name" data-i18n="approach.need2Name"></p>
            <p class="need-voice" data-i18n="approach.need2Voice"></p>
            <h3 data-i18n="approach.need2Q"></h3>
            <ul class="loop-points need-points">
              <li data-i18n="approach.need2P1"></li>
              <li data-i18n="approach.need2P2"></li>
              <li data-i18n="approach.need2P3"></li>
              <li data-i18n="approach.need2P4"></li>
            </ul>
          </article>
          <article class="need">
            <p class="need-name" data-i18n="approach.need3Name"></p>
            <p class="need-voice" data-i18n="approach.need3Voice"></p>
            <h3 data-i18n="approach.need3Q"></h3>
            <ul class="loop-points need-points">
              <li data-i18n="approach.need3P1"></li>
              <li data-i18n="approach.need3P2"></li>
              <li data-i18n="approach.need3P3"></li>
            </ul>
          </article>
        </div>

        <p class="needs-close">
          <span data-i18n="approach.needsClose"></span>
          <a href="connect.html" data-i18n="approach.needsLink"></a>
        </p>
      </section>

    </div>
  </section>"""),
    "2026AIinSecCLNLC.html": ("page.nlc26.title", """\
  <section class="section" id="nlc">
    <div class="wrap">
      <p class="eyebrow" data-i18n="nlc26.eyebrow">2026 AI in Sec CL NLC</p>
      <h1 class="page-title" data-i18n="nlc26.title">Splitting heaven and earth with an AI axe</h1>

      <!-- The title already says the axe splits heaven from earth. The banner
           says it a second time without words, which is why it carries no
           label and stays out of the language toggle: two flat ink blocks and
           one ochre, hatched where a gradient would otherwise be reached for. -->
      <div class="figure-hero" aria-hidden="true">
        <svg class="cut" viewBox="0 0 1200 260" preserveAspectRatio="xMidYMid meet" focusable="false">
          <!-- the ranges behind, pulled from the lighter block -->
          <path class="ink-3" d="M-20 225 L80 120 L190 175 L300 140 L400 225 Z"/>
          <path class="ink-3" d="M800 225 L900 140 L1010 175 L1120 120 L1220 225 Z"/>
          <!-- the lit gap between the two cut faces -->
          <rect class="och-w" x="520" y="36" width="160" height="189"/>
          <g class="line-o">
            <path d="M600 30 L600 10"/>
            <path d="M629 38 L639 20"/>
            <path d="M571 38 L561 20"/>
            <path d="M650 59 L667 49"/>
            <path d="M550 59 L533 49"/>
          </g>
          <circle class="och" cx="600" cy="88" r="44"/>
          <!-- one mountain, halved: each face left standing where it was cut -->
          <path class="ink" d="M110 225 L520 40 L520 225 Z"/>
          <path class="ink" d="M1090 225 L680 40 L680 225 Z"/>
          <g class="carve">
            <path d="M192 188 L238 204"/>
            <path d="M266 155 L312 171"/>
            <path d="M340 121 L386 137"/>
            <path d="M413 88 L459 104"/>
            <path d="M471 62 L509 75"/>
            <path d="M762 77 L716 93"/>
            <path d="M836 110 L790 126"/>
            <path d="M910 144 L864 160"/>
            <path d="M983 177 L937 193"/>
            <path d="M1041 203 L995 219"/>
          </g>
          <!-- water, cut as separate strokes the way a block would carry it -->
          <g class="line">
            <path d="M70 236 h160"/>
            <path d="M270 236 h100"/>
            <path d="M420 246 h230"/>
            <path d="M690 236 h130"/>
            <path d="M860 246 h250"/>
            <path d="M1130 236 h60"/>
          </g>
        </svg>
      </div>

      <!-- Order follows the session itself: what we are here to do, the
           practicalities, the thing being shown, how it was made, the tools
           behind it, then the two hours of hands-on, then the objections. -->
      <div class="work-layout">
        <nav class="project-nav" aria-label="On this page">
          <ul>
            <li><a href="#goals" data-i18n="nlc26.navGoals"></a></li>
            <li><a href="#session" data-i18n="nlc26.navSession"></a></li>
            <li><a href="#vocabsummit" data-i18n="nlc26.navVs"></a></li>
            <li><a href="#how-it-started" data-i18n="nlc26.navStory"></a></li>
            <li><a href="#evolution" data-i18n="nlc26.navEvo"></a></li>
            <li><a href="#stack" data-i18n="nlc26.navStack"></a></li>
            <li><a href="#hands-on" data-i18n="nlc26.navHands"></a></li>
            <li><a href="#concerns" data-i18n="nlc26.navQ"></a></li>
            <li><a href="#feedback" data-i18n="nlc26.navFb"></a></li>
          </ul>
        </nav>

        <div class="project-detail">

          <div class="doc-block" id="goals">
            <h2 class="section-title" data-i18n="nlc26.goalsTitle"></h2>
            <!-- Numbered, because four targets in order is what a sprint is.
                 1 and 2 frame; 3 and 4 are the prototype-then-iterate pair.
                 An <ol> here rather than the big circled .loop: at the top of
                 the page these are the goal, not a procedure to work through. -->
            <ol class="loop-points loop-points-num">
              <li data-i18n="nlc26.goal1"></li>
              <li data-i18n="nlc26.goal2"></li>
              <li data-i18n="nlc26.goal3"></li>
              <li data-i18n="nlc26.goal4"></li>
            </ol>
          </div>

          <div class="doc-block" id="session">
            <!-- Presenter first, logistics after: how a session programme
                 lists a session. Audience last, because it scopes the rest. -->
            <ul class="loop-points">
              <li data-i18n="nlc26.dSpeaker"></li>
              <li data-i18n="nlc26.dDate"></li>
              <li data-i18n="nlc26.dTime"></li>
              <li data-i18n="nlc26.dVenue"></li>
              <li data-i18n="nlc26.dAudience"></li>
            </ul>
          </div>

          <div class="doc-block" id="vocabsummit">
            <h2 class="section-title" data-i18n="nlc26.vsTitle"></h2>
            <!-- Profiles rather than classes: the platform left the three
                 pilot classes months ago, and one number that counts everyone
                 who has actually opened it says more than two that don't. -->
            <ul class="stats">
              <li><span class="value">3,889</span><span class="label" data-i18n="nlc26.statWords"></span></li>
              <li><span class="value">386</span><span class="label" data-i18n="nlc26.statProfiles"></span></li>
              <li><span class="value">353</span><span class="label" data-i18n="nlc26.statShips"></span></li>
              <li><span class="value">1</span><span class="label" data-i18n="nlc26.statPeople"></span></li>
            </ul>

            <div class="shots">
        <figure class="shot shot-wide">
          <div class="screen-bezel">
            <div class="screen-inner" data-missing="assets/screenshots/vocabsummit-landing.jpg">
              <img src="assets/screenshots/vocabsummit-landing.jpg" alt="" data-i18n="nlc26.shot1" data-i18n-attr="alt" decoding="async">
            </div>
          </div>
          <figcaption data-i18n="nlc26.shot1"></figcaption>
        </figure>
        <figure class="shot">
          <div class="screen-bezel">
            <div class="screen-inner" data-missing="assets/screenshots/vocabsummit-mountain.jpg">
              <img src="assets/screenshots/vocabsummit-mountain.jpg" alt="" data-i18n="nlc26.shot2" data-i18n-attr="alt" decoding="async">
            </div>
          </div>
          <figcaption data-i18n="nlc26.shot2"></figcaption>
        </figure>
        <figure class="shot">
          <div class="screen-bezel">
            <div class="screen-inner" data-missing="assets/screenshots/vocabsummit-dashboard.jpg">
              <img src="assets/screenshots/vocabsummit-dashboard.jpg" alt="" data-i18n="nlc26.shot3" data-i18n-attr="alt" decoding="async">
            </div>
          </div>
          <figcaption data-i18n="nlc26.shot3"></figcaption>
        </figure>
            </div>

            <p class="featured-cta">
              <a class="btn btn-primary" href="https://btvssclunit.github.io/VocabSummit"
                 data-i18n="nlc26.vsCta" target="_blank" rel="noopener">Open VocabSummit</a>
              <a class="btn btn-ghost" href="work.html" data-i18n="nlc26.vsWork">The project write-up</a>
              <a class="btn btn-ghost" href="practice.html" data-i18n="nlc26.vsApproach">How it was designed</a>
            </p>
          </div>

          <!-- A transition, and a disarming one: it began as one message. -->
          <div class="doc-block" id="how-it-started">
            <h2 class="section-title" data-i18n="nlc26.storyTitle"></h2>

            <!-- Hardcoded, not translated: a transcript, not copy. -->
            <div class="copy-block">
              <span class="cap" data-i18n="nlc26.storyCap"></span>
              <pre lang="en">I want to build a powerful secondary school revision tool on SLS
(student learning space) for students to revise all CL vocab covered
over 4 years, and for it to be trackable to let them see their progress,
so that they can revisit the vocab that they have yet to master.

I don't know how to start, could you help me?
https://www.learning.moe.edu.sg/teacher-user-guide/author/html5-content-development/</pre>
            </div>

            <ul class="loop-points">
              <li data-i18n="nlc26.storyP2"></li>
            </ul>

            <div class="highlight-box">
              <p data-i18n="nlc26.storyClose"></p>
            </div>

            <!-- The three moves the paragraph above names, drawn. The stamps
                 carry the pictures and the HTML carries the words, so the
                 language toggle keeps working inside the diagram. -->
            <ol class="cycle">
              <li>
                <svg class="cut cycle-glyph" viewBox="0 0 72 52" aria-hidden="true" focusable="false">
                  <path class="ink" d="M4 4 h64 v34 h-40 l-8 10 v-10 h-16 Z"/>
                  <g class="carve">
                    <path d="M14 15 h44"/>
                    <path d="M14 23 h44"/>
                  </g>
                  <path class="line-o" d="M14 31 h26"/>
                </svg>
                <span class="cycle-step">01</span>
                <span class="cycle-label" data-i18n="nlc26.cycle1"></span>
              </li>
              <li>
                <svg class="cut cycle-glyph" viewBox="0 0 72 52" aria-hidden="true" focusable="false">
                  <rect class="ink" x="4" y="5" width="64" height="42" rx="2"/>
                  <rect class="paper" x="10" y="11" width="52" height="30"/>
                  <circle class="och" cx="52" cy="19" r="4"/>
                  <path class="ink" d="M10 41 L26 24 L34 32 L45 19 L62 41 Z"/>
                </svg>
                <span class="cycle-step">02</span>
                <span class="cycle-label" data-i18n="nlc26.cycle2"></span>
              </li>
              <li>
                <svg class="cut cycle-glyph" viewBox="0 0 72 52" aria-hidden="true" focusable="false">
                  <rect class="paper" x="9" y="4" width="54" height="44"/>
                  <rect class="rule" x="9" y="4" width="54" height="44"/>
                  <g class="line">
                    <path d="M17 16 h38"/>
                    <path d="M17 34 h22"/>
                  </g>
                  <g class="line-o">
                    <path d="M17 25 h38"/>
                    <path d="M40 34 L46 41 L58 27"/>
                  </g>
                </svg>
                <span class="cycle-step">03</span>
                <span class="cycle-label" data-i18n="nlc26.cycle3"></span>
              </li>
            </ol>
            <p class="cycle-return" data-i18n="nlc26.cycleReturn"></p>
          </div>

          <!-- The gallery is its own block rather than a coda to the story,
               because it makes a separate claim: that the first version was
               bad, that this was fine, and that nothing here was arrived at
               in one go. The first version is embedded rather than described,
               since a teacher who can click it will believe it. -->
          <div class="doc-block" id="evolution">
            <h2 class="section-title" data-i18n="nlc26.evoTitle"></h2>
            <p class="section-sub" data-i18n="nlc26.evoLede"></p>

            <div class="evo" data-evo-root>
              <div class="evo-track" data-evo-track tabindex="0"
                   data-i18n-aria-label="nlc26.evoTitle" role="group">


                <figure class="evo-slide shot">
                  <div class="screen-bezel">
                    <div class="screen-inner" data-missing="assets/screenshots/evolution/02-2026-04-10-home.jpg">
                      <img src="assets/screenshots/evolution/02-2026-04-10-home.jpg" alt=""
                           data-i18n="nlc26.evo2" data-i18n-attr="alt" loading="lazy" decoding="async">
                    </div>
                  </div>
                  <figcaption class="evo-cap">
                    <span class="evo-date">2026.04.10</span>
                    <span class="evo-name" lang="zh-Hans">华文词语温习</span>
                    <span class="evo-note" data-i18n="nlc26.evo2"></span>
                  </figcaption>
                </figure>


                <figure class="evo-slide shot">
                  <div class="screen-bezel">
                    <div class="screen-inner" data-missing="assets/screenshots/evolution/04-2026-04-19-forge.jpg">
                      <img src="assets/screenshots/evolution/04-2026-04-19-forge.jpg" alt=""
                           data-i18n="nlc26.evo4" data-i18n-attr="alt" loading="lazy" decoding="async">
                    </div>
                  </div>
                  <figcaption class="evo-cap">
                    <span class="evo-date">2026.04.19</span>
                    <span class="evo-name" lang="zh-Hans">词王淬炼坊</span>
                    <span class="evo-note" data-i18n="nlc26.evo4"></span>
                  </figcaption>
                </figure>

                <figure class="evo-slide shot">
                  <div class="screen-bezel">
                    <div class="screen-inner" data-missing="assets/screenshots/evolution/05-2026-08-08-summit.jpg">
                      <img src="assets/screenshots/evolution/05-2026-08-08-summit.jpg" alt=""
                           data-i18n="nlc26.evo5" data-i18n-attr="alt" loading="lazy" decoding="async">
                    </div>
                  </div>
                  <figcaption class="evo-cap">
                    <span class="evo-date">2026.08.08</span>
                    <span class="evo-name" lang="zh-Hans">词山学海</span>
                    <span class="evo-note" data-i18n="nlc26.evo5"></span>
                  </figcaption>
                </figure>

                <figure class="evo-slide shot">
                  <div class="screen-bezel">
                    <div class="screen-inner" data-missing="assets/screenshots/evolution/06-today-summit.jpg">
                      <img src="assets/screenshots/evolution/06-today-summit.jpg" alt=""
                           data-i18n="nlc26.evo6" data-i18n-attr="alt" loading="lazy" decoding="async">
                    </div>
                  </div>
                  <figcaption class="evo-cap">
                    <span class="evo-date">2026.08.27</span>
                    <span class="evo-name" lang="zh-Hans">词山学海</span>
                    <span class="evo-note" data-i18n="nlc26.evo6"></span>
                  </figcaption>
                </figure>

              </div>

              <!-- Built by hand rather than by a library: six figures and two
                   buttons do not need one, and the strip still drags without
                   any of this. -->
              <div class="evo-controls">
                <button class="evo-btn" type="button" data-evo="prev"
                        data-i18n-aria-label="nlc26.evoPrev">
                  <svg class="cut" viewBox="0 0 12 20" aria-hidden="true" focusable="false">
                    <path class="line" d="M10 2 L2 10 L10 18" stroke-width="2.4"/>
                  </svg>
                </button>
                <button class="evo-btn" type="button" data-evo="next"
                        data-i18n-aria-label="nlc26.evoNext">
                  <svg class="cut" viewBox="0 0 12 20" aria-hidden="true" focusable="false">
                    <path class="line" d="M2 2 L10 10 L2 18" stroke-width="2.4"/>
                  </svg>
                </button>
                <ul class="evo-dots" data-evo-dots></ul>
                <span class="evo-count" data-evo-count></span>
              </div>
            </div>

            <!-- Not a screenshot. The actual file from 10 April, served as it
                 was uploaded, so nobody has to take the word "rough" on
                 trust. Loaded only when someone asks for it. -->
            <div class="evo-embed embed">
              <div class="screen-bezel">
                <div class="screen-inner">
                  <button class="embed-play" type="button"
                          data-embed-src="assets/archive/2026-04-10-first-version.html"
                          data-embed-title="华文词语温习 · 2026-04-10">
                    <span class="embed-play-icon" aria-hidden="true">&#9654;</span>
                    <span class="embed-play-label" data-i18n="nlc26.evoPlay"></span>
                  </button>
                </div>
              </div>
              <p class="embed-note" data-i18n="nlc26.evoPlayNote"></p>
            </div>

            <div class="highlight-box">
              <span class="label" data-i18n="nlc26.evoLabel"></span>
              <p data-i18n="nlc26.evoClose"></p>
            </div>
          </div>

          <div class="doc-block" id="stack">
            <h2 class="section-title" data-i18n="nlc26.stackTitle"></h2>
            <p class="section-sub" data-i18n="nlc26.stackLede"></p>

            <!-- The whole argument of this section is the direction of one
                 arrow, and an arrow is faster to look at than to read. The
                 five below are named by their own brands, so the diagram
                 needs no translated labels beyond the two on the arrow. -->
            <div class="roster">
              <div class="roster-you">
                <svg class="cut roster-glyph" viewBox="0 0 40 40" aria-hidden="true" focusable="false">
                  <circle class="ink" cx="20" cy="12" r="8"/>
                  <path class="ink" d="M4 38 a16 16 0 0 1 32 0 Z"/>
                </svg>
                <span class="roster-name" data-i18n="nlc26.rosterYou"></span>
                <span class="roster-role" data-i18n="nlc26.rosterYouRole"></span>
              </div>

              <div class="roster-flow">
                <svg class="cut roster-fan" viewBox="0 0 1000 132" aria-hidden="true" focusable="false">
                  <path class="line-o" d="M500 10 C 500 64, 95.5 64, 95.5 112" stroke-width="2" vector-effect="non-scaling-stroke"/>
                  <path class="line-o" d="M500 10 C 500 64, 297.8 64, 297.8 112" stroke-width="2" vector-effect="non-scaling-stroke"/>
                  <path class="line-o" d="M500 10 C 500 64, 500 64, 500 112" stroke-width="2" vector-effect="non-scaling-stroke"/>
                  <path class="line-o" d="M500 10 C 500 64, 702.2 64, 702.2 112" stroke-width="2" vector-effect="non-scaling-stroke"/>
                  <path class="line-o" d="M500 10 C 500 64, 904.5 64, 904.5 112" stroke-width="2" vector-effect="non-scaling-stroke"/>
                  <path class="och" d="M90.0 110 L101.0 110 L95.5 126 Z"/>
                  <path class="och" d="M292.3 110 L303.3 110 L297.8 126 Z"/>
                  <path class="och" d="M494.5 110 L505.5 110 L500 126 Z"/>
                  <path class="och" d="M696.7 110 L707.7 110 L702.2 126 Z"/>
                  <path class="och" d="M899.0 110 L910.0 110 L904.5 126 Z"/>
                  <circle class="och" cx="500" cy="10" r="4.5"/>
                </svg>
                <svg class="cut roster-stem" viewBox="0 0 26 40" aria-hidden="true" focusable="false">
                  <path class="line-o" d="M13 3 L13 27" stroke-width="2.4"/>
                  <path class="och" d="M13 38 L5 24 L21 24 Z"/>
                </svg>
              </div>

              <ul class="roster-tools">
                <li>
                  <svg class="cut roster-glyph" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
                    <path class="ink" d="M3 5 h26 v17 h-16 l-7 8 v-8 h-3 Z"/>
                    <g class="carve"><path d="M9 12 h14"/><path d="M9 17 h9"/></g>
                  </svg>
                  Claude
                </li>
                <li>
                  <svg class="cut roster-glyph" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
                    <rect class="ink" x="3" y="5" width="26" height="22" rx="2"/>
                    <g class="carve"><path d="M9 13 L14 17 L9 21"/><path d="M17 21 h7"/></g>
                  </svg>
                  Claude Code
                </li>
                <li>
                  <svg class="cut roster-glyph" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
                    <rect class="ink" x="3" y="6" width="26" height="20" rx="2"/>
                    <circle class="och" cx="11" cy="13" r="2.6"/>
                    <path class="paper" d="M6 26 L14 17 L19 22 L23 18 L27 26 Z"/>
                  </svg>
                  ChatGPT
                </li>
                <li>
                  <svg class="cut roster-glyph" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
                    <circle class="ink" cx="16" cy="16" r="13"/>
                    <g class="carve">
                      <path d="M3 16 h26"/>
                      <path d="M16 3 a17 17 0 0 1 0 26 a17 17 0 0 1 0 -26"/>
                    </g>
                  </svg>
                  GitHub
                </li>
                <li>
                  <svg class="cut roster-glyph" viewBox="0 0 32 32" aria-hidden="true" focusable="false">
                    <circle class="ink" cx="11" cy="16" r="7"/>
                    <circle class="paper" cx="11" cy="16" r="2.6"/>
                    <g class="line">
                      <path d="M17 16 h12" stroke-width="3"/>
                      <path d="M24 16 v5" stroke-width="3"/>
                      <path d="M29 16 v5" stroke-width="3"/>
                    </g>
                  </svg>
                  Firebase
                </li>
              </ul>
            </div>
            <p class="roster-note" data-i18n="nlc26.rosterNote"></p>
            <!-- The founder leads, because the ordering is the argument:
                 the five that follow are hired, and not one of them decides
                 anything. Two lines each, no more. -->
            <!-- The founder leads, because the ordering is the argument:
                 the five that follow are hired, and not one of them decides
                 anything. One line each: the role and the who are already in
                 the heading, so the line only has to say what they do. -->
            <ol class="loop">
        <li>
          <h3 data-i18n="nlc26.p1Title"></h3>
          <p class="loop-cycle" data-i18n="nlc26.p1Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.p1A"></li>
          </ul>
        </li>
        <li>
          <div class="tool-head">
            <h3 data-i18n="nlc26.p2Title"></h3>
            <a class="tool-link" href="https://claude.ai"
               target="_blank" rel="noopener">claude.ai</a>
          </div>
          <p class="loop-cycle" data-i18n="nlc26.p2Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.p2A"></li>
          </ul>
          <details class="tool-guide">
            <summary><span data-i18n="nlc26.guideLabel"></span></summary>
            <p class="tool-guide-body">
              <a href="https://support.claude.com/en/articles/9517075-what-are-projects"
                 target="_blank" rel="noopener" data-i18n="nlc26.p2Guide"></a>
            </p>
          </details>
        </li>
        <li>
          <div class="tool-head">
            <h3 data-i18n="nlc26.p3Title"></h3>
            <a class="tool-link" href="https://claude.com/product/claude-code"
               target="_blank" rel="noopener">claude.com/claude-code</a>
          </div>
          <p class="loop-cycle" data-i18n="nlc26.p3Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.p3A"></li>
          </ul>
          <details class="tool-guide">
            <summary><span data-i18n="nlc26.guideLabel"></span></summary>
            <p class="tool-guide-body">
              <a href="https://docs.claude.com/en/docs/claude-code/quickstart"
                 target="_blank" rel="noopener" data-i18n="nlc26.p3Guide"></a>
            </p>
          </details>
        </li>
        <li>
          <div class="tool-head">
            <h3 data-i18n="nlc26.p4Title"></h3>
            <a class="tool-link" href="https://chatgpt.com"
               target="_blank" rel="noopener">chatgpt.com</a>
          </div>
          <p class="loop-cycle" data-i18n="nlc26.p4Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.p4A"></li>
          </ul>
          <details class="tool-guide">
            <summary><span data-i18n="nlc26.guideLabel"></span></summary>
            <p class="tool-guide-body">
              <a href="https://help.openai.com/en/articles/8932459-creating-images-in-chatgpt"
                 target="_blank" rel="noopener" data-i18n="nlc26.p4Guide"></a>
            </p>
          </details>
        </li>
        <li>
          <div class="tool-head">
            <h3 data-i18n="nlc26.p5Title"></h3>
            <a class="tool-link" href="https://github.com"
               target="_blank" rel="noopener">github.com</a>
            <a class="tool-link" href="https://pages.github.com"
               target="_blank" rel="noopener">pages.github.com</a>
          </div>
          <p class="loop-cycle" data-i18n="nlc26.p5Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.p5A"></li>
            <li data-i18n="nlc26.p5B"></li>
          </ul>
          <details class="tool-guide">
            <summary><span data-i18n="nlc26.guideLabel"></span></summary>
            <p class="tool-guide-body">
              <a href="https://docs.github.com/en/pages/quickstart"
                 target="_blank" rel="noopener" data-i18n="nlc26.p5Guide"></a>
            </p>
          </details>
        </li>
        <li>
          <div class="tool-head">
            <h3 data-i18n="nlc26.p6Title"></h3>
            <a class="tool-link" href="https://firebase.google.com"
               target="_blank" rel="noopener">firebase.google.com</a>
          </div>
          <p class="loop-cycle" data-i18n="nlc26.p6Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.p6A"></li>
          </ul>
          <details class="tool-guide">
            <summary><span data-i18n="nlc26.guideLabel"></span></summary>
            <p class="tool-guide-body">
              <a href="https://firebase.google.com/docs/auth/web/start"
                 target="_blank" rel="noopener" data-i18n="nlc26.p6Guide"></a>
            </p>
          </details>
        </li>
            </ol>
          </div>

          <!-- Four stages, not six steps. Stage 1 is the floor everyone leaves
               with; stages 2 to 4 are differentiation, and they are named for
               what changes rather than for who got that far. The cycle labels
               borrow VocabSummit's own chapter components, so the session and
               the platform it is about speak the same vocabulary. -->
          <div class="doc-block" id="hands-on">
            <h2 class="section-title" data-i18n="nlc26.handsTitle"></h2>
            <p class="section-sub" data-i18n="nlc26.handsLede"></p>
            <p class="lede" data-i18n="nlc26.handsFloor"></p>

            <!-- The four stages as a climb, because that is what the platform
                 they are building calls it. Numerals only: the stage titles
                 are in the ordered list below, which is also what a screen
                 reader gets, so the map itself is decoration. -->
            <div class="climb" aria-hidden="true">
              <svg class="cut" viewBox="0 0 1080 220" preserveAspectRatio="xMidYMid meet" focusable="false">
                <path class="ink-3" d="M0 220 L180 148 L330 178 L520 96 L700 138 L880 56 L1080 108 L1080 220 Z"/>
                <path class="line-o" d="M60 185 C 220 175 250 120 380 128 C 520 136 540 80 700 86 C 850 92 880 50 1010 48"
                      stroke-dasharray="9 9" stroke-width="2.5"/>
                <!-- the summit marker, the only ochre on the map -->
                <path class="och" d="M1010 48 L1010 14 L1042 24 L1010 34"/>
                <g class="ink">
                  <circle cx="60" cy="185" r="21"/>
                  <circle cx="380" cy="128" r="21"/>
                  <circle cx="700" cy="86" r="21"/>
                  <circle cx="1010" cy="48" r="21"/>
                </g>
                <g fill="var(--paper)" font-family="var(--font-serif)" font-size="21"
                   text-anchor="middle" font-weight="600">
                  <text x="60" y="193">1</text>
                  <text x="380" y="136">2</text>
                  <text x="700" y="94">3</text>
                  <text x="1010" y="56">4</text>
                </g>
              </svg>
            </div>

            <p class="featured-cta">
              <a class="btn btn-primary" href="https://github.com/kaixinbuilds/VocabClimbStarter"
                 data-i18n="nlc26.handsCta" target="_blank" rel="noopener">Open the starter</a>
              <a class="btn btn-ghost" href="https://claude.ai" data-i18n="nlc26.handsClaude"
                 target="_blank" rel="noopener">claude.ai</a>
            </p>

            <ol class="loop">
        <li>
          <h3 data-i18n="nlc26.s1Title"></h3>
          <p class="loop-cycle" data-i18n="nlc26.s1Cycle"></p>
          <!-- Every step that names a page carries the page: a teacher
               following along on a laptop should never have to retype a
               URL, and the upload step is shown rather than described. -->
          <ul class="loop-points">
            <li class="step-item">
              <span data-i18n="nlc26.s1P1"></span>
              <a class="tool-link" href="https://github.com/signup"
                 target="_blank" rel="noopener">github.com/signup</a>
            </li>
            <li class="step-item">
              <span data-i18n="nlc26.s1P2"></span>
              <a class="tool-link" href="https://github.com/new"
                 target="_blank" rel="noopener">github.com/new</a>
            </li>
            <li class="step-item">
              <span data-i18n="nlc26.s1P3"></span>
              <a class="tool-link" href="https://github.com/kaixinbuilds/VocabClimbStarter"
                 target="_blank" rel="noopener">github.com/kaixinbuilds/VocabClimbStarter</a>
            </li>
            <li>
              <span data-i18n="nlc26.s1P4"></span>
              <figure class="shot step-shot">
                <div class="screen-bezel">
                  <div class="screen-inner" data-missing="assets/screenshots/github-upload-files.png">
                    <img src="assets/screenshots/github-upload-files.png" alt=""
                         data-i18n="nlc26.s1Shot" data-i18n-attr="alt" decoding="async">
                  </div>
                </div>
                <figcaption data-i18n="nlc26.s1Shot"></figcaption>
              </figure>
            </li>
            <li data-i18n="nlc26.s1P5"></li>
            <li data-i18n="nlc26.s1P6"></li>
          </ul>
        </li>
        <li>
          <h3 data-i18n="nlc26.s2Title"></h3>
          <p class="loop-cycle" data-i18n="nlc26.s2Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.s2P1"></li>
            <li data-i18n="nlc26.s2P2"></li>
          </ul>

          <div class="copy-block">
            <span class="cap" data-i18n="nlc26.promptCap"></span>
            <pre lang="zh-Hans">我是中学华文老师，不会编码。
我已经把一个小游戏的整个资料夹和一份词语表格上传到这个 Project 里。

请帮我做三件事：
1. 读我上传的表格，把里面的词语和释义，照 my-words.js 原本的格式
   重新写成一个完整的 my-words.js 给我下载。格式不要动，只换内容。
2. 把 UNIT_NAME 改成：〔你的单元名称〕
3. 把 SECONDS 改成：〔90〕

改好之后直接给我可以下载的 my-words.js，并告诉我放回哪个位置。

如果我的表格有问题（缺了释义、词语重复、少于 8 个），请先告诉我，不要自己猜。</pre>
          </div>

          <ul class="loop-points">
            <li data-i18n="nlc26.s2P3"></li>
            <li data-i18n="nlc26.s2P4"></li>
          </ul>
        </li>
        <li>
          <h3 data-i18n="nlc26.s3Title"></h3>
          <p class="loop-cycle" data-i18n="nlc26.s3Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.s3P1"></li>
            <li data-i18n="nlc26.s3P2"></li>
          </ul>

          <!-- The three art files only. Editing the colours in index.html
               means editing the game's own source, which is a different kind
               of risk from dropping in a picture, and this stage is not the
               place to introduce it. -->
          <div class="copy-block">
            <span class="cap" data-i18n="nlc26.promptCap"></span>
            <pre lang="zh-Hans">我已经把这个游戏的整个资料夹上传到这个 Project 里了。

告诉我 art 资料夹里那三张图（landscape.jpg、wall.jpg、climber.png）
分别是做什么用的、该做多大，我想自己换掉。</pre>
          </div>
        </li>
        <li>
          <h3 data-i18n="nlc26.s4Title"></h3>
          <p class="loop-cycle" data-i18n="nlc26.s4Cycle"></p>
          <ul class="loop-points">
            <li data-i18n="nlc26.s4P1"></li>
            <li data-i18n="nlc26.s4P2"></li>
            <li data-i18n="nlc26.s4P3"></li>
          </ul>

        </li>
            </ol>

            <!-- One box, not three: everything you reach for when it goes wrong. -->
            <div class="highlight-box">
              <span class="label" data-i18n="nlc26.stuckLabel"></span>
              <ul class="loop-points">
                <li data-i18n="nlc26.stuck1"></li>
                <li data-i18n="nlc26.stuck2"></li>
                <li data-i18n="nlc26.stuck3"></li>
                <li data-i18n="nlc26.stuck4"></li>
                <li data-i18n="nlc26.stuck5"></li>
                <li data-i18n="nlc26.stuck6"></li>
              </ul>
            </div>
          </div>

          <div class="doc-block" id="concerns">
            <h2 class="section-title" data-i18n="nlc26.qTitle"></h2>
            <ol class="loop">
        <li>
          <h3 data-i18n="nlc26.q1"></h3>
          <ul class="loop-points">
            <li data-i18n="nlc26.q1A"></li>
            <li data-i18n="nlc26.q1B"></li>
            <li data-i18n="nlc26.q1C"></li>
          </ul>
        </li>
        <li>
          <h3 data-i18n="nlc26.q2"></h3>
          <ul class="loop-points">
            <li data-i18n="nlc26.q2A"></li>
            <li data-i18n="nlc26.q2B"></li>
            <li data-i18n="nlc26.q2C"></li>
          </ul>
        </li>
        <li>
          <h3 data-i18n="nlc26.q3"></h3>
          <ul class="loop-points">
            <li data-i18n="nlc26.q3A"></li>
            <li data-i18n="nlc26.q3B"></li>
            <li data-i18n="nlc26.q3C"></li>
          </ul>
        </li>
        <li>
          <h3 data-i18n="nlc26.q4"></h3>
          <ul class="loop-points">
            <li data-i18n="nlc26.q4A"></li>
            <li data-i18n="nlc26.q4B"></li>
          </ul>
        </li>
            </ol>

          </div>

          <!-- Last block on the page, and deliberately so: the ask comes after
               the concerns have been answered, not before. A heading and one
               button: the form states its own terms on the first screen, so
               repeating them here would only delay the tap. -->
          <div class="doc-block" id="feedback">
            <h2 class="section-title" data-i18n="nlc26.fbTitle"></h2>

            <p class="featured-cta">
              <a class="btn btn-primary" href="https://docs.google.com/forms/d/e/1FAIpQLSdx0yJJJ5u6PAiAwNQQ-1_y4BrDkbNJyNMgMFDphnFu42w6vA/viewform"
                 data-i18n="nlc26.fbCta" target="_blank" rel="noopener">Open the feedback form</a>
            </p>

            <!-- The community's own motto, quoted back to it, set as a colophon.
                 It closes the page, so it moves with the last block rather than
                 staying with the concerns. -->
            <p class="epigraph colophon">
              <span class="zh" lang="zh-Hans">地上本无路，走的人多了，便成了路</span>
              <span class="en" lang="en">The earth had no roads to begin with. When enough
                people walk the same way, a road appears.</span>
              <span class="src" data-i18n="nlc26.epigraphSrc"></span>
            </p>
          </div>

        </div>
      </div>
    </div>
  </section>"""),
    "contact.html": ("page.contact.title", """\
  <section class="section" id="contact">
    <div class="wrap">
      <h1 class="page-title" data-i18n="contact.title">Get in touch</h1>
      <p class="page-sub" data-i18n="contact.subtitle"></p>

      <div class="contact-layout">
        <div class="contact-details">
          <p class="contact-direct">
            <span class="contact-label" data-i18n="contact.emailLabel"></span>
            <a class="contact-email" href="mailto:kai_xin_chun@moe.edu.sg">kai_xin_chun@moe.edu.sg</a>
          </p>
        </div>

        <!-- Hung and labelled the way a print would be in a gallery, rather
             than used as decoration. Hides itself if the image is missing. -->
        <figure class="specimen">
          <div class="specimen-plate">
            <img src="assets/art/prints/payphone-block-print.png" alt=""
                 data-i18n="specimen.alt" data-i18n-attr="alt" decoding="async">
          </div>
          <figcaption class="specimen-label">
            <span class="specimen-title" data-i18n="specimen.title"></span>
            <span class="specimen-artist" data-i18n="specimen.artist"></span>
            <span class="specimen-meta" data-i18n="specimen.meta"></span>
          </figcaption>
        </figure>
      </div>

      <!-- The name, glossed. 观象制器 carries an allusion not every reader
           will catch, so the last page states it plainly. Same device as the
           epigraphs, set as a colophon: it closes a page instead of opening
           one. -->
      <p class="epigraph colophon">
        <span class="zh" lang="zh-Hans">观象制器</span>
        <span class="en" lang="en">Observe the pattern, then make the tool.</span>
        <span class="src" data-i18n="colophon.source"></span>
      </p>
    </div>
  </section>"""),
}


def render(page):
    title_key, body = PAGES[page]
    nav = "\n".join(
        '        <a href="%s"%s data-i18n="%s">%s</a>'
        % (href, CURRENT if href == page else "", key, label)
        for href, key, label in NAV)

    static_title, static_desc = META[page]
    canonical = SITE + "/" + ("" if page == "index.html" else page)

    html = (HEAD
            .replace("__TITLE_KEY__", title_key)
            .replace("__STATIC_TITLE__", static_title)
            .replace("__STATIC_DESC__", static_desc)
            .replace("__CANONICAL__", canonical)
            .replace("__SHARE_IMAGE__", SHARE_IMAGE)
            .replace("__GA__", GA_SNIPPET.replace("__GA_ID__", GA_MEASUREMENT_ID)
                                if GA_MEASUREMENT_ID else "")
            .replace("__HOME__", CURRENT if page == "index.html" else "")
            .replace("__NAV__", nav + "\n"))
    return html.replace("<!DOCTYPE html>", "<!DOCTYPE html>\n" + BANNER, 1) \
        + '<main id="main">\n' + body + "\n</main>" + TAIL


def write_sitemap():
    """A sitemap and a robots.txt, so search engines can find every page and
    know where the sitemap lives. No lastmod: it would change on every build
    whether or not the content did, for no real benefit."""
    urls = "\n".join(
        "  <url><loc>%s</loc><priority>%s</priority></url>"
        % (SITE + "/" + ("" if p == "index.html" else p), "1.0" if p == "index.html" else "0.8")
        for p in PAGES)
    pathlib.Path("sitemap.xml").write_text(
        '<?xml version="1.0" encoding="UTF-8"?>\n'
        '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'
        + urls + "\n</urlset>\n")
    pathlib.Path("robots.txt").write_text(
        "User-agent: *\n"
        "Allow: /\n\n"
        "Sitemap: %s/sitemap.xml\n" % SITE)
    # Every page here is finished HTML. Jekyll has nothing to contribute, and
    # a Jekyll build that fails takes the whole deploy down with it, silently:
    # Pages keeps serving the last good commit and the site simply stops
    # updating. This empty file turns Jekyll off, so Pages just serves files.
    pathlib.Path(".nojekyll").write_text("")


def check_i18n():
    """Every key the pages ask for must exist, and every nlc26 key must be asked
    for. A page that references a deleted key renders the key name at readers,
    which shipped once and was seen. This makes that a build failure instead."""
    i18n = json.loads(pathlib.Path("i18n.json").read_text())
    asked = set()
    for page in PAGES:
        html = pathlib.Path(page).read_text()
        asked |= set(re.findall(r'data-i18n(?:-aria-label)?="([^"]+)"', html))

    missing = sorted(k for k in asked if k not in i18n)
    # Only nlc26 is checked for orphans: the other namespaces are shared across
    # pages and rendered from JSON, so an unused key there is not a mistake.
    orphans = sorted(k for k in i18n
                     if k.startswith("nlc26.") and k not in asked)

    if missing:
        print("\nBUILD FAILED. These keys are on a page but not in i18n.json:")
        for k in missing:
            print("   ", k)
        print("\nEither add them to i18n.json or take them out of build.py.")
        raise SystemExit(1)
    if orphans:
        print("\nUnused nlc26 keys still in i18n.json (harmless, but tidy up):")
        for k in orphans:
            print("   ", k)
    print("i18n keys check out")


if __name__ == "__main__":
    for page in PAGES:
        pathlib.Path(page).write_text(render(page))
        print("built", page)
    write_sitemap()
    print("built sitemap.xml, robots.txt and .nojekyll")
    check_i18n()
